import { makePostgrestClient, PostgrestClient } from '../src';
...
describe('#PostgrestClient', () => {
  ...
  describe('#Utils', () => {
    test('clone', () => {
      const client = makePostgrestClient().url('http://localhost:3000');
    ...
    test('toQuery', () => {
      const client = makePostgrestClient().url('http://localhost:3000').model('city');
  ...
  describe('#Horizontal filtering (rows)', () => {
    test('Simple condition', () => {
      const client = makePostgrestClient().url('http://localhost:3000').model('city').filter({
        test: {
  ...
  describe('#Vertical filtering (columns)', () => {
    test('Simple select', () => {
      const client = makePostgrestClient().url('http://localhost:3000').model('city').select(['test']);

      expect(client.toQuery()).toEqual({ url: 'http://localhost:3000/city', queryString: { select: 'test' } });
    });

    test('Multiple select', () => {
      const client = makePostgrestClient().url('http://localhost:3000').model('city').select(['test', 'test2']);
    ...
    test('Select with alias and cast', () => {
      const client = makePostgrestClient()
        ...
        .model('city')
        .select([{ column: 'test_test', alias: 'testTest', cast: 'text' }, 'test2']);
  ...
  describe('#Ordering', () => {
    test('Empty order', () => {
      const client = makePostgrestClient().url('http://localhost:3000').model('city').order([]);
    ...
    test('Order by multiple columns', () => {
      const client = makePostgrestClient().url('http://localhost:3000').model('city').order(['test_aaa', 'test_bbb']);
